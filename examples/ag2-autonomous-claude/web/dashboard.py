...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  /* ---- hunter stat sheet (modal) ---- */
  .overlay{position:fixed;inset:0;background:rgba(2,4,10,.74);backdrop-filter:blur(3px);
     display:none;place-items:center;z-index:60;padding:18px;}
  .overlay.show{display:grid;}
  .sheet{width:min(560px,96vw);max-height:90vh;overflow:auto;position:relative;padding:24px;
     background:linear-gradient(180deg,rgba(13,22,46,.97),rgba(8,12,26,.97));
     border:1px solid var(--cyan);border-radius:18px;box-shadow:0 0 44px rgba(56,224,255,.35);}
  .sheet h3{margin:0 0 2px;font-size:24px;letter-spacing:.04em;}
  .sheet .close{position:absolute;top:14px;right:14px;width:34px;height:34px;cursor:pointer;font-size:17px;
     background:transparent;border:1px solid var(--line);color:var(--muted);border-radius:9px;}
  .sheet .close:hover,.sheet .close:focus-visible{color:var(--cyan);border-color:var(--cyan);outline:none;}
  .sheet .verdict{margin:14px 0 16px;font-size:15px;line-height:1.6;}
  .bd{margin:10px 0;} .bd .l{display:flex;justify-content:space-between;font-size:11px;letter-spacing:.1em;
     text-transform:uppercase;color:var(--muted);margin-bottom:4px;}
  .sheet .stat{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--line);font-size:14px;}
  .row-btn{cursor:pointer;} .row-btn:focus-visible{outline:2px solid var(--cyan);outline-offset:-2px;}
  .cbot{display:flex;gap:10px;padding:9px 0;border-bottom:1px solid var(--line);font-size:13px;align-items:flex-start;}
  .cbot:last-child{border-bottom:0;}
  .cav{width:32px;height:32px;border:1px solid var(--line);border-radius:9px;display:grid;place-items:center;position:relative;flex:0 0 auto;background:#141b2a;}
  .cav .cb{position:absolute;bottom:-6px;right:-6px;font-size:11px;background:#0e131e;border-radius:50%;}
  .cbot b{color:var(--cyan);}
...
$("roster").innerHTML=hunters.map(h=>{const plw=Math.min(100,Math.round(h.s));
  return `<tr class="row-btn" tabindex="0" role="button" aria-label="Open ${esc(h.sym)} stat sheet" data-sym="${esc(h.sym)}"><td><span class="rk ${h.rk}">${h.rk}</span></td>
...
$("market").innerHTML=hunters.map(h=>{
  ...
  return `<div class="panel mcard row-btn" tabindex="0" role="button" aria-label="Open ${esc(h.sym)} stat sheet" data-sym="${esc(h.sym)}"><div style="display:flex;justify-content:space-between;align-items:center">
...
// ----- hunter stat sheet (modal) -----
function parts(v){return {
  cover:v.payout<=40?30:v.payout<=60?24:v.payout<=75?15:v.payout<=90?6:0,
  debt: v.de<=50?25:v.de<=100?18:v.de<=150?10:v.de<=250?4:0,
  rec:  v.years>=25?25:v.years>=10?18:v.years>=5?10:v.years>=1?4:0,
  yld:  v.yield<=5?20:v.yield<=7?12:v.yield<=10?5:0 };}
function verdictText(h){
  const concern=h.payout>90?"its payout leaves almost no cushion":h.de>200?"its debt load is heavy":
    h.yield>8?"its very high yield can signal a cut":h.years<5?"its dividend record is short":"no single glaring red flag";
  const tier=(h.rk==='S'||h.rk==='A')?"a sturdy long-term dividend hunter":h.rk==='B'?"a decent holding worth watching":"a fragile pick — handle with care";
  return `Rank ${h.rk} — ${tier}. Biggest watch-item: ${concern}. (A metaphor for dividend safety, not a buy signal.)`;
}
function council(h){return [
  ["Buffett","💰",(h.payout<=60&&h.years>=10)?"Earnings comfortably cover a long-rising dividend — the durable kind I like.":(h.payout>90)?"The payout leaves almost no cushion; I'd want more cover.":"Decent — I'd check the moat protects these earnings over a decade."],
  ["Munger","🧠",(h.de>150||h.payout>90)?"Invert it: high debt and a stretched payout are what snap a dividend. Avoid that combo.":"Few obvious ways this breaks — low-ish debt and a sane payout."],
  ["Lynch","🛒",(h.yield>8)?"A yield that high usually means the market expects a cut. Be careful.":"Understandable and steady — just don't overpay for it."],
  ["Graham","🛡️",(h.years>=20)?"A long, unbroken record — the defensive investor's friend.":(h.years<5)?"Too short a record to lean on yet.":"A fair record; insist on a margin of safety."],
  ["Risk","⚠️",(h.yield>8)?"That yield smells like a trap — size it very small.":(h.de>250)?"Heavy debt — a downturn could force a cut.":"Diversify and size it small regardless."],
  ["Analyst","📊","Score "+h.s+"/100 — "+(h.s>=70?"sturdy":h.s>=45?"watch it":"fragile")+". Make it one holding among many; nothing here is guaranteed."],
];}
function bd(label,pts,max){const w=Math.round(pts/max*100);
  return `<div class="bd"><div class="l"><span>${label}</span><span>${pts}/${max}</span></div>
    <div class="bar"><i style="width:${w}%"></i></div></div>`;}
let lastFocus=null;
function openSheet(sym){
  const h=hunters.find(x=>x.sym===sym); if(!h) return;
  const p=parts(h); const k=h.hist||[]; let spark="";
  if(k.length>1){const W=512,H=70,pad=4,mn=Math.min(...k),mx=Math.max(...k),sp=(mx-mn)||1;
    const pts=k.map((x,i)=>{const px=pad+(W-2*pad)*i/(k.length-1);const py=pad+(H-2*pad)*(1-(x-mn)/sp);
      return px.toFixed(1)+","+py.toFixed(1);}).join(" ");
    spark=`<svg viewBox="0 0 512 70" preserveAspectRatio="none" style="width:100%;height:70px" role="img" aria-label="${esc(h.sym)} price trend"><polyline points="${pts}" fill="none" stroke="#38e0ff" stroke-width="2.5"/></svg>`;}
  $("sheet").innerHTML=`<button class="close" id="sheetClose" aria-label="Close">✕</button>
    <div style="display:flex;align-items:center;gap:14px">
      <span class="rank-badge" style="width:46px;height:46px;font-size:20px">${h.rk}</span>
      <div><h3 id="sheetTitle">${esc(h.sym)}</h3><div class="ptitle">${esc(h.name)}</div></div>
      <div style="margin-left:auto;text-align:right"><div class="k" style="font-size:11px;color:var(--muted);letter-spacing:.2em">POWER</div>
        <div style="font-size:22px;font-weight:800">${fmt(h.pw)}</div></div>
    </div>
    <p class="verdict">${verdictText(h)}</p>
    ${spark}
    <h2 class="t" style="margin-top:18px">Power Breakdown</h2>
    ${bd("Payout coverage",p.cover,30)}${bd("Debt safety",p.debt,25)}
    ${bd("Dividend record",p.rec,25)}${bd("Yield (trap?) safety",p.yld,20)}
    <h2 class="t" style="margin-top:18px">🤖 Mentor Council</h2>
    ${council(h).map(c=>`<div class="cbot"><span class="cav">🤖<span class="cb">${c[1]}</span></span><div><b>${esc(c[0])}-bot</b><div>${esc(c[2])}</div></div></div>`).join("")}
    <h2 class="t" style="margin-top:18px">Raw Stats</h2>
    <div class="stat"><span>Price</span><span>$${h.price??'—'}</span></div>
    <div class="stat"><span>Dividend yield</span><span>${h.yield.toFixed(2)}%</span></div>
    <div class="stat"><span>Payout ratio</span><span>${h.payout}%</span></div>
    <div class="stat"><span>Debt-to-equity</span><span>${h.de}</span></div>
    <div class="stat"><span>Years of dividend growth</span><span>${h.years}</span></div>
    <div class="stat"><span>Safety score</span><span>${h.s}/100</span></div>`;
  lastFocus=document.activeElement;
  $("overlay").classList.add('show');
  $("sheetClose").focus();
  $("sheetClose").onclick=closeSheet;
}
function closeSheet(){ $("overlay").classList.remove('show'); if(lastFocus) lastFocus.focus(); }
document.addEventListener('click',e=>{
  const r=e.target.closest('[data-sym]'); if(r){openSheet(r.dataset.sym);return;}
  if(e.target.id==='overlay') closeSheet();
});
document.addEventListener('keydown',e=>{
  if(e.key==='Escape') closeSheet();
  const r=e.target.closest && e.target.closest('[data-sym]');
  if(r && (e.key==='Enter'||e.key===' ')){e.preventDefault();openSheet(r.dataset.sym);}
});
